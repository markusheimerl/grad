...
int main() {
    ...
    // y_class is of shape [num_samples x output_dim] (flattened).
    ...
    // EVALUATION.
    // For each sample and for each output dim, determine the predicted bin (argmax over each group of num_bins),
    // then convert that bin into a continuous prediction via unbin_value.
    // Compute overall classification accuracy (percentage of correct predictions) and
    // mean absolute error (averaged over all outputs).
    int correct = 0;
    double sum_abs_error = 0.0;
    for (int i = 0; i < num_samples; i++) {
        for (int d = 0; d < output_dim; d++) {
            int offset = i * (num_bins * output_dim) + d * num_bins;
            int predicted_bin = 0;
    ...
    // Print sample predictions (for the first 5 samples, each with all outputs).
