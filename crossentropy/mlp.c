/* mlp.c – Adapted for classification with continuous evaluation using the true output range

   This file assumes data.h supplies data functions (generate_synthetic_data, save_data_to_csv, embed_input)
   as well as INPUT_RANGE_MIN and INPUT_RANGE_MAX values for generating raw inputs.
     
   Workflow:
     1. generate_synthetic_data produces raw inputs X and continuous targets y.
     2. We iterate over y to determine the true minimum and maximum output values.
     3. We compute discrete labels (y_class) by mapping each continuous target in [out_min, out_max]
        into one of target_bins.
     4. The network (from mlp.h) is trained using cross‑entropy loss on these discrete labels.
     5. During evaluation, we convert the predicted bin (via argmax) into a continuous prediction
        by taking the bin’s center (based on the true output range) and compare it to y.
*/
...
#include <math.h>
#include <time.h>
#include "../data.h"   // Provides generate_synthetic_data, save_data_to_csv, and INPUT_RANGE_MIN/MAX
#include "mlp.h"    // Contains network definitions and functions (including embed_input)
...
int main() {
    ...
    const int output_dim = 1;            // Our synthetic data produces one continous output.
    
    // Embedding lookup parameters (for input features).
    const int embedding_num_bins = 10;   // Number of bins per feature for the embedding lookup.
    const int embedding_dim = 8;         // Dimension of each embedding vector.
    
    // MLP parameters.
    const int hidden_dim = 1024;
    const int target_bins = 256;          // Number of discrete bins (classes) used for the target.
    const int batch_size = num_samples;  // Full-batch training.
    
    //----------------------------------------------------------------------------
    // DATA GENERATION
    //----------------------------------------------------------------------------
    float *X, *y;  // y will hold continuous target values.
    generate_synthetic_data(&X, &y, num_samples, raw_input_dim, output_dim);
    
    // Determine the true minimum and maximum of the continuous outputs.
    ...
    //----------------------------------------------------------------------------
    // Compute discrete labels: map each continuous target in [out_min, out_max] into a bin.
    //----------------------------------------------------------------------------
    int *y_class = (int*)malloc(num_samples * sizeof(int));
    for (int i = 0; i < num_samples; i++) {
        // Since output_dim==1, each target is y[i].
    ...
    // Allocate memory for the embedded representation.
    ...
    for (int epoch = 0; epoch < num_epochs; epoch++) {
        ...
        // Compute cross‑entropy loss; calculate_loss uses y_class to compute gradients.
    ...
    //----------------------------------------------------------------------------
    // EVALUATION:
    // For each sample, determine the predicted bin from the network's logits (argmax),
    // then compute a continuous prediction using the bin's center based on [out_min, out_max].
    // Also, compute discrete classification accuracy.
    //----------------------------------------------------------------------------
    int correct = 0;
    ...
    for (int i = 0; i < num_samples; i++) {
        ...
        // Convert the predicted bin to a continuous prediction.
