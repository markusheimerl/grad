...
////////////////////////////////////////////////////////////////////////////////
/*
  embed_input: Maps each raw input (shape: [batch_size x input_dim]) into a token
  by quantizing each feature and copying its embedding vector. The result is stored
  in net->layer_input[0] (and also copied to the provided embedded_output).
*/
...
/*
  ...
  Each layer does:
    ...
    – Feed–forward: computes hidden = ReLU(r * W_ff1); then f = hidden * W_ff2;
       final output = r + f.
  The final output is copied into net->predictions.
*/
void forward_pass(Net* net) {
    ...
    // Copy final layer output into predictions.
    memcpy(net->predictions, net->layer_input[net->num_layers], total_tokens * d_dim * sizeof(float));
}

/*
  calculate_loss: For each token in each sample, applies softmax to the logits
  and computes the average cross–entropy loss. Also it sets net->error.
*/
float calculate_loss(Net* net, int* target_labels) {
    float loss = 0.0f;
    int batch = net->batch_size;
    int classes = net->num_bins;  // Each token’s output has length = num_bins.
...
/*
  zero_gradients: Resets the gradients for all layers.
*/
...
/*
  ...
  For each layer the feed–forward branch gradient is computed then the self–attention
  branch gradient is computed. The gradients for W_Q, W_K, W_V, W_ff1 and W_ff2 are accumulated.
  (Note: For clarity the backprop derivations mirror those in the single–layer code.)
*/
void backward_pass(Net* net) {
    ...
    // dX holds the gradient w.r.t. the output of the current layer.
    float* dX = (float*)malloc(total_tokens * d_dim * sizeof(float));
    memcpy(dX, net->error, total_tokens * d_dim * sizeof(float)); // starting from final output
    ...
    for (int l = net->num_layers - 1; l >= 0; l--) {
        ...
        // We now backprop attention per sample.
        float* dV_total = (float*)calloc(total_tokens * d_dim, sizeof(float));
        float* dQ_total = (float*)calloc(total_tokens * d_dim, sizeof(float));
        float* dK_total = (float*)calloc(total_tokens * d_dim, sizeof(float));
        for (int s = 0; s < net->batch_size; s++) {
            float* scores = net->attn_scores_layers[l] + s * tokens * tokens;
...
int main() {
    ...
    const int embedding_dim = num_bins; // Embedding dimension (equal to num_bins).
